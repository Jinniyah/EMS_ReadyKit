...
//   northcentralus — primary region for all application resources
//   centralus      — required for Azure Static Web Apps (not available in northcentralus)
...
resource "azurerm_subscription_policy_assignment" "allowed_locations" {
  ...
  description          = "Restricts deployments to northcentralus (primary) and centralus (required for Static Web Apps)."
...
resource "azurerm_policy_definition" "deny_public_ip" {
  ...
  metadata = jsonencode({ category = "EMS ReadyKit Security" })

  policy_rule = jsonencode({
    if = {
      allOf = [{ field = "type", equals = "Microsoft.Network/publicIPAddresses" }]
    }
    then = { effect = "Deny" }
  })
}
