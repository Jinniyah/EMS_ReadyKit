...
def _station(db: Session, *, name: Optional[str] = None) -> Station:
    s = Station(
        name=name or f"Station-{_uid()}", address="100 Main St", region="Downriver"
    )
    db.add(s)
    db.flush()
    return s
...
class TestStationEndpoints:
    ...
    def test_list_stations_returns_active_by_default(self, client, auth_admin):
        ...
        client.post(
            "/api/v1/stations",
            json={"name": name_a, "address": "1 St", "region": "R"},
            headers=auth_admin,
        )
        client.post(
            "/api/v1/stations",
            json={"name": name_i, "address": "2 St", "region": "R", "active": False},
            headers=auth_admin,
        )
    ...
    def test_get_station_by_id(self, client, auth_admin):
        r = client.post(
            "/api/v1/stations",
            json={"name": f"Station-{_uid()}", "address": "1 St", "region": "R"},
            headers=auth_admin,
        )
...
class TestVehicleEndpoints:
    ...
    def test_create_vehicle_auto_creates_location(self, client, db, auth_admin):
        ...
        response = client.post(
            "/api/v1/vehicles",
            json={"station_id": sid, "vehicle_number": vnum, "vehicle_type": "BLS"},
            headers=auth_admin,
        )
        assert response.status_code == 201
    ...
    def test_list_station_vehicles(self, client, auth_admin):
        r1 = client.post(
            "/api/v1/stations",
            json={"name": f"S1-{_uid()}", "address": "1 St", "region": "R"},
            headers=auth_admin,
        )
        r2 = client.post(
            "/api/v1/stations",
            json={"name": f"S2-{_uid()}", "address": "2 St", "region": "R"},
            headers=auth_admin,
        )
        ...
        client.post(
            "/api/v1/vehicles",
            json={"station_id": s1_id, "vehicle_number": vnum1, "vehicle_type": "ALS"},
            headers=auth_admin,
        )
        client.post(
            "/api/v1/vehicles",
            json={"station_id": s2_id, "vehicle_number": vnum2, "vehicle_type": "ALS"},
            headers=auth_admin,
        )
...
class TestItemEndpoints:
    ...
    def test_list_items_filter_controlled(self, client, auth_admin, station_id):
        ...
        response = client.get(
            "/api/v1/items?controlled_substance=true", headers=auth_admin
        )
        assert response.status_code == 200
...
class TestInventoryEndpoints:
    ...
    def test_list_expiring_lots(self, client, auth_admin):
        ...
        assert response.status_code == 200
        assert all(
            date.fromisoformat(lot["expiration_date"])
    ...
    def test_list_location_stock(self, client, auth_admin):
        ...
        response = client.get(
            f"/api/v1/inventory/locations/{loc_id}/stock", headers=auth_admin
        )
        assert response.status_code == 200
    ...
    def test_list_location_par_levels(self, client, auth_admin):
        ...
        response = client.get(
            f"/api/v1/inventory/locations/{loc_id}/par-levels", headers=auth_admin
        )
        assert response.status_code == 200
        assert len(response.json()) >= 1
...
class TestCompartmentEndpoints:
    ...
    def test_list_compartments_sorted_by_sort_order(self, client, auth_admin):
        ...
        for name, order in [
            ...
            client.post(
                f"/api/v1/inventory/locations/{loc_id}/compartments",
                json={"location_id": loc_id, "name": name, "sort_order": order},
                headers=auth_admin,
            )
        response = client.get(
    ...
    def test_get_compartment_by_id(self, client, auth_admin):
        ...
        cr = client.post(
            f"/api/v1/inventory/locations/{loc_id}/compartments",
            json={"location_id": loc_id, "name": "First Out Bag", "sort_order": 0},
            headers=auth_admin,
        )
        cid = cr.json()["compartment_id"]
        response = client.get(
            f"/api/v1/inventory/compartments/{cid}", headers=auth_admin
        )
        assert response.status_code == 200
...
class TestCheckLineItems:
    ...
    def _setup(self, client, auth_admin):
        ...
        cr = client.post(
            f"/api/v1/inventory/locations/{loc_id}/compartments",
            json={"location_id": loc_id, "name": "Compartment #1", "sort_order": 1},
            headers=auth_admin,
        )
        cid = cr.json()["compartment_id"]
        ir = client.post(
    ...
    def test_wrong_lot_item_returns_422(self, client, auth_admin):
        ...
        assert response.status_code == 422
        assert (
            "lot_id" in response.json()["detail"].lower()
            or "item" in response.json()["detail"].lower()
        )
...
class TestCheckEndpoints:
    ...
    def test_multiple_checks_same_vehicle_same_day_all_succeed(
        self, client, auth_admin
    ):
        sid, vid = self._make_station_and_vehicle(client, auth_admin)
        payload_base = {
        ...
        r1 = client.post(
            "/api/v1/checks/daily",
            json={**payload_base, "timestamp": _utcnow(), "notes": "Shift-start check"},
            headers=auth_admin,
        )
        ...
        r3 = client.post(
            "/api/v1/checks/daily",
            json={**payload_base, "timestamp": _utcnow(), "notes": "Shift-end check"},
            headers=auth_admin,
        )
    ...
    def test_station_compliance_today_returns_all_checks(self, client, auth_admin):
        ...
        response = client.get(
            f"/api/v1/checks/daily/station/{sid}/today", headers=auth_admin
        )
        assert response.status_code == 200
...
class TestStationDateRangeChecks:
    ...
    def test_date_range_returns_checks_in_window(self, client, auth_admin):
        ...
        response = client.get(
            f"/api/v1/checks/daily/station/{sid}?from=2026-05-01&to=2026-05-31",
            headers=auth_admin,
        )
        assert response.status_code == 200
    ...
    def test_from_after_to_returns_422(self, client, auth_admin):
        ...
        response = client.get(
            f"/api/v1/checks/daily/station/{sid}?from=2026-05-31&to=2026-05-01",
            headers=auth_admin,
        )
        assert response.status_code == 422
    ...
    def test_range_over_90_days_returns_422(self, client, auth_admin):
        sid, _ = self._make_sv(client, auth_admin)
        response = client.get(
            f"/api/v1/checks/daily/station/{sid}?from=2026-01-01&to=2026-12-31",
            headers=auth_admin,
        )
        assert response.status_code == 422
        assert "90" in response.json()["detail"]

    def test_responder_can_query_own_station(
        self, client, db, auth_admin, auth_responder
    ):
        sid, vid = self._make_sv(client, auth_admin)
        _add_member(db, sid, "test-responder@ems.local", "Responder")
...
class TestCSCheckEndpoints:
    ...
    def test_create_cs_check_same_signers_returns_422(self, client, db, auth_responder):
        sr = client.post(
            "/api/v1/stations",
            json={"name": f"S-{_uid()}", "address": "1 St", "region": "R"},
            headers={"Authorization": "Bearer test-administrator"},
        )
        sid = sr.json()["station_id"]
        vr = client.post(
            "/api/v1/vehicles",
        ...
        response = client.post(
            ...
            json={
                "vehicle_id": vid,
                "secondary_signer": "Test Responder",
                "timestamp": _utcnow(),
                "discrepancy_flag": False,
            },
            headers=auth_responder,
    ...
    def test_cs_check_discrepancy_creates_high_severity_audit(
        ...
        client.post(
            ...
            json={
                ...
                "notes": "Count off by 1.",
            },
            headers=auth_admin,
        )
        event = (
            db.query(AuditEvent)
            .filter(AuditEvent.action == "CS_DISCREPANCY", AuditEvent.vehicle_id == vid)
            .first()
        )
        assert event is not None
    ...
    def test_cs_check_no_discrepancy_creates_info_audit(self, client, db, auth_admin):
        ...
        event = (
            db.query(AuditEvent)
            .filter(
                AuditEvent.action == "CS_CHECK_COMPLETED", AuditEvent.vehicle_id == vid
            )
            .first()
        )
        assert event is not None
...
class TestAuditEndpoints:
    ...
    def test_list_audit_events_filter_by_action(self, client, auth_admin):
        ...
        response = client.get(
            "/api/v1/audit?action=CHECK_COMPLETED", headers=auth_admin
        )
        assert response.status_code == 200
        assert len(response.json()) >= 1
...
class TestCheckTypes:
    ...
    def _submit_check(
        self, client, auth_admin, *, sid, vid, cid, check_date: str, line_items: list
    ) -> dict:
        response = client.post(
            "/api/v1/checks/daily",
            json={
        ...
        ), f"Check submission failed: {response.json()}"
    ...
    def test_mixed_check_types_worst_case_determines_overall_status(
        self, client, auth_admin
    ):
        sid, vid, _loc_id, cid = self._make_env(client, auth_admin)
    ...
    def test_low_measurement_and_ok_supply_yields_needs_restock(
        self, client, auth_admin
    ):
        sid, vid, _loc_id, cid = self._make_env(client, auth_admin)
