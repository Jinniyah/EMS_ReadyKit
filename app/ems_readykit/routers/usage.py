...
POST /checks/usage
  Record items used during/after a call from a vehicle or portable location.
  Creates a UsageEvent audit trail for the history view.
  Does NOT decrement station supply room stock -- supply room is only
  decremented during check wizard reconcile (SR-B4 in checks.py) when the
  responder physically tops off the vehicle from the supply room.
  The logged quantities are subtracted from get_last_readings in checks.py
  so the check wizard pre-fills correctly and flags shortages automatically.
...
def _build_event_read(event: UsageEvent) -> UsageEventRead:
    return UsageEventRead(
        ...
        location_label=event.location.label if event.location else None,
...
@router.post(
    ...
    summary="Log items used after a call (vehicle or portable location)",
)
def create_usage_event(
    ...
    # Only SUPPLY items can be logged -- others are silently skipped.
    non_supply = [
    ...
    # NOTE: supply room stock is NOT decremented here.
    # Items were consumed from the vehicle/jump bag, not the supply room.
    # Supply room decrements happen in _auto_decrement_supply_room (checks.py)
    # when a vehicle check shows quantities were topped off during reconcile.
