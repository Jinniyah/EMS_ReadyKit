"""
routers/admin_items.py
Item catalog and par level admin endpoints.

Supervisor+ can create and edit items and par levels.
Administrators only: deactivate items, update AI fields.
...
Routes (all prefixed /admin via router):
  ...
  GET    /admin/items/import/template          Download CSV template
  ...
  POST   /admin/items/{id}/assign              Assign item to compartment
  PATCH  /admin/par-levels/{id}                Edit par level min/max
  PATCH  /admin/par-levels/{id}/deactivate     Soft-deactivate par level
...
@router.patch(
    ...
    response_model=ItemRead,
    summary="Update AI identification fields -- Admin only (AI-B1)",
)
...
@router.get(
    ...
    summary="Active assignment count for an item (ADMIN-F4)",
)
...
def assign_item_to_compartment(
    ...
    db.refresh(par)
    logger.info(
        "Par level assigned: par_id=%s item_id=%s",
        par.par_id,
        item_id,
        extra={
            "action": "PAR_ASSIGNED",
...
def _do_deactivate_par_level(
    par_id: int, actor: str, db: Session, reason: Optional[str] = None
) -> None:
    par = _get_par_or_404(par_id, db)
    if not par.active:
...
@router.patch(
    ...
    summary="Soft-deactivate a par level assignment (ADMIN-B8)",
)
...
@router.get(
    ...
    response_model=List[ParLevelAssignment],
    summary="List item par levels for a compartment",
)
