...
COMPARTMENTS = [
    {
        "name": "PC 1 (Airway)",
        "location_descriptor": "Interior, left side front",
        "sort_order": 1,
    },
    {
        "name": "PC 2 (Cardiac)",
        "location_descriptor": "Interior, left side mid",
        "sort_order": 2,
    },
    {
        "name": "PC 3 (Trauma)",
        "location_descriptor": "Interior, right side front",
        "sort_order": 3,
    },
    {
        "name": "Drug Bag",
        "location_descriptor": "Interior, centre console",
        "sort_order": 4,
        "restriction_note": "ALS crews only",
    },
    {
        "name": "Driver Side EC 1",
        "location_descriptor": "Exterior, driver side",
        "sort_order": 5,
    },
]
...
def main() -> None:
    ...
    try:
        ...
        existing = (
            db.query(Vehicle).filter(Vehicle.vehicle_number == VEHICLE_NUMBER).first()
        )

        if existing:
            print(
                f"Vehicle {VEHICLE_NUMBER} already exists (id={existing.vehicle_id}) — skipping vehicle creation."
            )
            vehicle = existing
        else:
            vehicle = Vehicle(
        ...
        if not location:
            print(
                f"ERROR: No inventory location found for vehicle {vehicle.vehicle_id}"
            )
            sys.exit(1)
