...
[![Tests — 396 passing](https://img.shields.io/badge/tests-396%20passing-brightgreen?logo=pytest&logoColor=white)](app/tests/)
...
| **O2 PSI validation** | Readings below minimum (500 PSI) are flagged LOW -- the truck does not get a passing check with insufficient oxygen |
...
| **Compliance calendar** | Week and month views showing check history per vehicle; tap any cell to open the check detail |
...
| **Station administration** | Manage membership -- control which crew members access which station's data |
| **Full audit trail** | Every material action logged with actor, timestamp, and entity -- legally defensible |
| **Item catalog** | Manage inventory item list; bulk import via CSV with template download; AI identification fields |
| **Par level assignment** | Assign items to compartments with min/restock quantities; mark priority items; deactivate with reason |
...
| Domain modeling | Station to Vehicle to Compartment to Item hierarchy; 6 check types; 7 line item statuses |
| Data integrity | SQLAlchemy 2.0, Alembic (24 migrations applied), DB-level constraints |
| Audit trail | First-class audit events with actor, entity, severity, and metadata; immutable records |
| Security | OWASP Top 10 reviewed; 0 known CVEs (pip-audit in CI); security headers split correctly across API and SWA; rate limiting; production hardening |
| Testing | 396 automated tests -- persona-based (Jamie/Earl/Jennifer), safety-critical (O2 PSI, AED dates), seed integrity, RBAC, retirement, usage log |
...
| **Supervisor** | All Responder actions plus: view all station checks; manage repair lifecycle; manage vehicles and compartments; manage station membership; create and edit items; correct supply counts; expiring items alerts |
| **Administrator** | Full system access -- all Supervisor capabilities plus station creation; item deactivation; par level deactivation; audit access; vehicle and location retirement; station retirement; hard-delete management |
...
python seed.py        # seeds Unit 712, Newberg Township, Marcellus Township, TEST station
pytest tests/ -q
...
2. `pytest` -- all 396 tests must pass
