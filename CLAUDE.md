...
# Last updated: 2026-07-19 — Session AS continued past the ONBOARD-1/ONBOARD-2 close-out (see prior entry, now superseded) to also ship F-5G3a: the compliance CSV export for Bobby. New `GET /checks/daily/station/{id}/export` (check_history.py, Supervisor+, 400-day cap distinct from the interactive 90-day-capped list endpoint) streams Simplified (one row/check) or Detailed (one row/line-item + a Controlled Substance Checks section for ALS dual-signature checks, joined via vehicle_id since that table has no station_id/check_id) CSVs, filtered by whole-station/vehicle/jump-bag/supply-room, with a CSV formula-injection guard on free-text columns and a CHECK_HISTORY_EXPORTED audit event. Frontend: new ExportPanel.jsx in Check History's All Checks tab with two direct download buttons (not a hidden toggle, per explicit correction mid-session). 29 new backend tests (591 passing) + 16 new frontend tests (249 passing), ruff/eslint clean. Found and fixed a real React bug while writing frontend tests — a hand-rolled useEffect that included its own fetched-data/loading-flag in its dependency array self-cancelled before ever clearing the loading state; refactored onto the existing useApi hook, which avoids this via ref-based cancellation (see architectural-decisions table below — a lesson worth applying to any future hand-rolled data-fetching effect). Not yet committed/pushed as of this write-up. Next: VALID-1 (live UAT validation of the export) per the locked backlog sequence.
...
| Item deactivation guard (ONBOARD-2) | `deactivate_item` (`admin_items.py`) blocks with 409 if any active `ParLevel` still references the item — same `(item_id, active=True)` count query as `count_item_assignments` (ADMIN-F4), so the two can never disagree. Message is deliberately generic (never names the other station/vehicle) since the caller may have no visibility into where the assignment actually lives. |
| Fire-engine `VehicleType` | Fire engine trucks reuse the existing `VehicleType.QRV` enum value — no dedicated enum value, no migration (Session AS/ONBOARD-1 decision). Apply the same convention when Newberg's own fire trucks are onboarded later. |
| Fire-truck/shared catalog items | Fire-truck check items (tire pressure/tread, portable pump, SCBA, generators, gas meter, gauge/light FUNCTIONAL checks) live in the shared `BASE_ITEM_SEED`, `category_group="Vehicle Operations"`, `station_supply=False` — every station's catalog picks them up via `seed_station_catalog()` even before that station has a fire truck onboarded (Session AS/ONBOARD-1). Be aware this also means any *other* new items added to `BASE_ITEM_SEED` bleed into every station's catalog the same way, whether intended or not. |
| Compliance CSV export (F-5G3a) | `GET /checks/daily/station/{id}/export` (`check_history.py`) — a *sibling* endpoint to the interactive `GET /checks/daily/station/{id}` list, not an extension of it; the interactive endpoint keeps its 90-day cap and JSON shape, the export endpoint gets its own 400-day cap and CSV shape. `whole_station=true` deliberately includes retired vehicles/locations (historical compliance data must not disappear because equipment was later retired) — retirement filtering belongs only in the frontend's *selectable* checkbox list, never in this kind of historical query (same `active`-vs-`retired_at` distinction as `BUG-AD1`). Free-text CSV cells (notes, corrective actions) get a leading-`'` formula-injection guard (OWASP CSV-injection mitigation) — apply the same guard to any future CSV export carrying real user-entered text. |
| **`useEffect` self-cancellation footgun** | **Never include a piece of state (or a loading flag) that the effect itself sets in that same effect's dependency array**, when the effect also uses a `cancelled`-flag-based cleanup pattern. Setting the state re-triggers the effect (dependency changed), which runs *that invocation's own cleanup* — flipping its `cancelled` flag to `true` — before the async chain's own `.then()/.finally()` gets a chance to run, silently dropping whatever state update it was going to make (permanently stranding a loading spinner, in the case that surfaced this — `ExportPanel.jsx`, Session AS/F-5G3a). This codebase already has the correct fix for "only fetch when a condition holds": the shared `useApi` hook (`shared/hooks/useApi.js`), which cancels via a `mountedRef` (a ref, not state) and takes the *condition* itself as a dependency (e.g. `useApi(() => cond ? fetchThing() : Promise.resolve(null), [cond, ...])`) rather than hand-rolling a `useEffect`. Prefer `useApi` over a custom effect for this shape of problem; if a custom effect is truly unavoidable, never let it depend on values only it produces. |
