...
# Last updated: 2026-06-12 (Session V addendum: dead code cleanup, par-level active filter, CI audit fix)
...
| UAT-BUG8 | Deactivated par level blocks check wizard — "1 remaining" stuck | `GET /inventory/locations/{id}/par-levels` returned ALL par levels including inactive ones (`active=False`). Deactivated item was not in `itemMap` (items endpoint filters active only), so ItemRow rendered null — invisible but still counted in `untouchedCount`. Fixed: added `ParLevel.active` filter to `list_location_par_levels` in `inventory.py`. | 2026-06-12 |
| CLEANUP-1 | Dead code audit pass | Deleted `RestockVehiclePanel.jsx` (orphaned since SR-F6) and `StockSummaryView.jsx` (never imported). Removed deprecated `getStationChecksToday` from `checkHistoryApi.js` (zero callers). `admin.py` par-level deactivate: extracted `_do_deactivate_par_level` helper shared by PATCH and DELETE endpoints; both now call `write_audit_event` (was using bare `logger.info`, violating CLAUDE.md audit rule); both now bind `current_user` for audit actor; PATCH now accepts optional `reason` field. CODEBASE_INDEX.md cleaned of resolved flagged items. | 2026-06-12 |
| CI-FIX1 | npm audit blocking CI on dev-tool CVEs | `deploy.yml` `npm audit --audit-level=high` → `--omit=dev`. Two high-severity esbuild CVEs (GHSA-gv7w-rqvm-qjhr, GHSA-g7r4-m6w7-qqqr) are dev-only: one fires only when running the dev server, the other is a Deno/build-registry concern. Neither is present in the deployed static bundle. Upgrading to vite@8 would also require vitest and @vitejs/plugin-react major bumps — deferred post-launch. | 2026-06-12 |
